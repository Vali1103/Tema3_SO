...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	// Inițializarea mutexului
	rc = pthread_mutex_init(&tp->queue_mutex, NULL);
	DIE(rc != 0, "pthread_mutex_init");

    // Inițializarea barierei
    // Numărul de thread-uri care trebuie să ajungă la barieră este num_threads
	rc = pthread_barrier_init(&tp->pool_barrier, NULL, num_threads);
...
void destroy_threadpool(os_threadpool_t *tp)
{
	...
	// distrug elementele de sincronizare ale threadpool-ului la sfarsit
	pthread_mutex_destroy(&tp->queue_mutex);
	pthread_barrier_destroy(&tp->pool_barrier);
